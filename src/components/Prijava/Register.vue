<template>
  <div>
    <div
      class="mx-auto container mt-5 rounded-lg register_1"
      ...
      <transition name="bounce">
        <div>
          <form novalidate @submit.prevent="register">
            <p style="text-style:Segoe UI" class="h4 text-center mb-5">
              Registracija
            </p>
            <div class="grey-text mx-4">
              ...
              <mdb-input
                v-model="lozinka"
                label="Lozinka"
                v-model.trim="$v.lozinka.$model"
                type="password"
                required
                invalidFeedback="Molimo unesite ispravnu lozinku"
              >
                <div class="error" v-if="!$v.lozinka.minLength">
                  Lozinka mora imati najmanje
                  {{ $v.lozinka.$params.minLength.min }} znakova.
                </div>
              </mdb-input>
            </div>
            <span class="text-center" v-if="showError"
              ><p style="color:red;">E-mail već postoji</p></span
            >
            <span class="text-center" v-if="showError1"
              ><p style="color:red;">Ispunite sva polja</p></span
            >
            <span class="text-center" v-if="showError2"
              ><p style="color:red;">Lozinka je neispravna</p></span
            >
            ...
            <div class="text-center">
              <mdb-btn type="sumbit" class="rounded-lg"
                >Registriraj se!</mdb-btn
              >
              <br /><br />
